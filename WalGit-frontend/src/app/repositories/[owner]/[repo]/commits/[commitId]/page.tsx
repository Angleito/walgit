  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-ocean-50/30 to-white dark:from-dark-300 dark:to-dark-100 relative">
      ...
      <div className="container px-4 py-8">
        <div className="mb-6">
          <Link href={`/repositories/${owner}/${repo}/commits`} className="inline-flex items-center text-sm text-ocean-600 hover:text-ocean-700 mb-4">
            ...
            <CardContent>
              <div className="flex items-center justify-between text-sm text-muted-foreground border-t border-border/60 pt-4">
                ...
                {commit.parentCommitId && (
                  <span>Parent <Link href={`/repositories/${owner}/${repo}/commits/${commit.parentCommitId}`} className="text-ocean-600 hover:underline"><code className="text-xs bg-muted px-1 py-0.5 rounded">{commit.parentCommitId.substring(0,9)}...</code></Link></span>
                )}
              </div>
            </CardContent>
          </Card>
