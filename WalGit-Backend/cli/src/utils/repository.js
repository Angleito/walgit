...
export const createRepository = async (options) => {
  ...
  // Create default .walgitignore file
  WorkingCopyManager.createDefaultIgnoreFile(process.cwd());
...
export const createCommit = async (options) => {
  ...
  try {
    ...
    const lastSnapshot = await workingCopy.getLastSnapshot();
    
    // Detect changes in the working copy
    const changes = await workingCopy.detectChanges(lastSnapshot);
    
    // Get all files to be committed
    const allFiles = await workingCopy.getAllFiles();
    const filesToCommit = allFiles.map(filePath => ({
      path: filePath,
      status: 'modified' // Simplified status
    }));
    
    progressSpinner.text = `Processing ${filesToCommit.length} files...`;
    ...
    // Save the current snapshot for future change detection
    const currentSnapshot = await workingCopy.snapshot();
    await workingCopy.saveSnapshot(currentSnapshot);
...
export const getRepositoryStatus = async (options) => {
  ...
  // In "always-staged" paradigm, all changes are considered staged for commit
  const allChanges = [...changes.added, ...changes.modified, ...changes.deleted];
  const stagedChanges = allChanges.map(file => ({
    path: file,
    status: changes.added.includes(file) ? 'new file' : 
            changes.deleted.includes(file) ? 'deleted' : 'modified'
  }));

  // With "always-staged" paradigm, there are no unstaged changes
  const unstagedChanges = [];
  
  // Untracked files are those ignored by .walgitignore
  const untrackedFiles = [];

  // Count commits ahead/behind
  // This is simplified - in a real implementation would compare with remote
  const commits = stagedChanges.length > 0 ? 1 : 0;
  const relation = commits > 0 ? 'ahead of' : 'up to date with';
...
export const getCurrentBranch = async () => {
  ...
  const headPath = path.join(walgitDir, 'HEAD');
  if (!fs.existsSync(headPath)) {
    return 'main'; // Default to main if HEAD doesn't exist
  }
  ...
  // If HEAD is detached (direct commit hash), return the hash
  return headContent;
};
...
/**
 * Repository class for managing repository metadata
 */
export class Repository {
  constructor(repoPath) {
    this.repoPath = repoPath;
    this.configPath = path.join(repoPath, '.walgit', 'config.json');
  }
  ...
  getRepoName() {
    ...
    return config.name || path.basename(this.repoPath);
  }
   ...
   */
  async setEncryptionEnabled(enabled, policyId = null) {
    const config = this.getConfig();
    config.encryptionEnabled = enabled;
    if (policyId) {
      config.encryptionPolicyId = policyId;
    }
    this.saveConfig(config);
  }

  /**
   * Get encryption policy ID
   * @returns {string|null} Encryption policy ID
   */
  async getEncryptionPolicyId() {
    const config = this.getConfig();
    return config.encryptionPolicyId || null;
  }

  /**
   * Check if repository is encrypted
   * @returns {boolean} Whether repository is encrypted
   */
  isEncrypted() {
    const config = this.getConfig();
    return config.encryptionEnabled || false;
  }

  /**
   * Get all blob IDs in repository
   * @returns {Array} Array of blob objects with ID and size
   */
  async getAllBlobs() {
    const blobs = [];
    const objectsDir = path.join(this.repoPath, '.walgit', 'objects');
    
    if (!fs.existsSync(objectsDir)) {
      return blobs;
    }

    // Simple implementation - in reality, you'd traverse the object store
    const files = fs.readdirSync(objectsDir);
    for (const file of files) {
      const stat = fs.statSync(path.join(objectsDir, file));
      if (stat.isFile()) {
        blobs.push({
          id: file,
          size: stat.size
  ...
  /**
   * Update blob ID reference
   * @param {string} oldId - Old blob ID
   * @param {string} newId - New blob ID
   */
  async updateBlobId(oldId, newId) {
    // Update references in the repository metadata
    // This would update commit objects, tree objects, etc.
    const config = this.getConfig();
    if (config.blobMapping) {
      config.blobMapping[oldId] = newId;
    } else {
      config.blobMapping = { [oldId]: newId };
    }
    this.saveConfig(config);
